After a Game Boy emulator in Rust ([GMBoy](https://github.com/mxmgorin/gmboy)) and a CHIP-8 interpreter in Go ([ch8go](https://github.com/mxmgorin/ch8go)), I got curious about the other side of the process. Both of those projects run bytecode that already exists, and the natural next step was to write a program that produces that bytecode itself. On top of that, I wanted to pick the simplest possible language, so the focus would fall on the compiler rather than on the parser and the syntax.
